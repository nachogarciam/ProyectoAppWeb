 ...
 * @author user
 */
@WebServlet(name = "ObtenJuegos1", urlPatterns = {"/ObtenJuegos1"})
public class ObtenJuegos1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Videojuego> lista = fachada.consultarVideojuegos();
        List<ArticuloED> lista2 =fachada.consultarInventarioVideojuegos();
        
        List listaUnidades=new ArrayList();
        int i=0;
        for (Videojuego juego : lista) {
            for (ArticuloED articulo : lista2) {
                if(juego.getNumCatalogo().equals(articulo.getArticulo().getNumCatalogo())){
                    listaUnidades.add(String.valueOf(articulo.getExistencia()));
                }else{
                    listaUnidades.add("0");
                }
            }
        ...
        request.setAttribute("listaJuegos2", listaUnidades);
