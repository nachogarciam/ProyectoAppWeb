...
public class ActualizarCliente extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        cliente.setNumCredencial((String) request.getParameter("num"));

        if (session.getAttribute("accionSel").equals("obtenActualizar")) {
            request.setAttribute("cliente", fachada.obten(cliente));

            session.setAttribute("accionSel", "actualizar");

            rd = request.getRequestDispatcher("actualizarCliente.jsp");

            rd.forward(request, response);
        } else {
            cliente.setNumCredencial((String) request.getParameter("numC"));
            cliente.setNombre((String) request.getParameter("titulo"));
            cliente.setDireccion((String) request.getParameter("genero"));
            cliente.setTelefono((String) request.getParameter("clasificacion"));
